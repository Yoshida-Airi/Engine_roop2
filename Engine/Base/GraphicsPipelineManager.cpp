...
void GraphicsPipelineManager::Initialize()
{
	...
	hr = dxCommon_->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&graphicPipelineState));
	assert(SUCCEEDED(hr));

	D3D12_GRAPHICS_PIPELINE_STATE_DESC graphicsPipelineStateDesc2{};
	graphicsPipelineStateDesc2.pRootSignature = particleRootSignature.Get();
	...
	//RTV情報
	graphicsPipelineStateDesc2.NumRenderTargets = 1;
	graphicsPipelineStateDesc2.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;
	//利用する形状（三角形
	graphicsPipelineStateDesc2.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
	graphicsPipelineStateDesc2.SampleDesc.Count = 1;
	graphicsPipelineStateDesc2.SampleMask = D3D12_DEFAULT_SAMPLE_MASK;


	hr = dxCommon_->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc2, IID_PPV_ARGS(&PgraphicPipelineState));
	assert(SUCCEEDED(hr));
...
void GraphicsPipelineManager::SetupRootSignature()
{
...
void GraphicsPipelineManager::SetupParticleRootSignature()
{
	D3D12_ROOT_SIGNATURE_DESC descriptionRootSignature{};
	descriptionRootSignature.Flags = D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT;
...
void GraphicsPipelineManager::SetupShader()
{
	...
	PvertexShaderBlob = CompileShader(L"Resources/Shaders/Particle.VS.hlsl", L"vs_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(PvertexShaderBlob != nullptr);
	PpixelShaderBlob = CompileShader(L"Resources/Shaders/Particle.PS.hlsl", L"ps_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(PpixelShaderBlob != nullptr);
}
